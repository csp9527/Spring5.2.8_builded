...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	// 用来表示一个bean的实例化需要依靠另一个bean先实例化，对应bean属性depend-on
	@Nullable
	private String[] dependsOn;
	/*
	该属性设置为false，这样容器在查找自动装配时，将不考虑该bean，即它不会被考虑作为其他bean自动
	装配的选项，但是该bean本身还是可以使用自动装配来注入其他bean的
	 */
	private boolean autowireCandidate = true;
	// 自动装配时当出现多个bean候选者时，将作为首选者，对应bean属性peimary
	...
	// 记录构造函数注入属性，对应bean属性contructor-arg
	@Nullable
	...
	// 定义这个bean的应用，APPLICATION：用户，INFRASTRUCTURE；完全内部使用，SUPPORT：某些复杂配置的一部分，程序设置
